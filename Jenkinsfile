	...
	stage('6. 制品仓库') {
		...
		dir("${app_name}"){
			nexusArtifactUploader artifacts: [
			    [artifactId: 'xxl-job-admin', classifier: 'SNAPSHOT', 
				file: './xxl-job-admin/target/xxl-job-admin-2.2.1-SNAPSHOT.jar', type: 'jar'], 

			    [artifactId: 'xxl-job-executor', classifier: 'SNAPSHOT', 
				file: './xxl-job-executor-samples/xxl-job-executor-sample-springboot/target/xxl-job-executor-sample-springboot-2.2.1-SNAPSHOT.jar',
				type: 'jar']], 
			    credentialsId: 'nexus-password', groupId: 'com.xuxueli', nexusUrl: 'nexus.huaweicloud.com', 
			    nexusVersion: 'nexus3', protocol: 'http', repository: 'test-env', version: 'release-${BUILD_NUMBER}'
		}
            }
        }
	stage('7.docker打包推送') {
